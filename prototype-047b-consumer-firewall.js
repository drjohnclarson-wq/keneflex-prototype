...
function translateKnownLegacy(){
 ...
 const packet=document.getElementById('kfxVPacketValue');if(packet){
   ...
   text(packet.querySelector('p'),'Your personalized actions, product-use guidance, movement, recovery and follow-up are already assembled for you.');
   const b=packet.querySelector('button');text(b,'View / save my Keneflex Plan →');
   const price=packet.querySelector('.kfxVPacketPrice');if(price)price.style.display='none';
   const chips=packet.querySelector('.kfxVPacketChips');if(chips)chips.style.display='none';
 }
 document.querySelectorAll('#solutionView [data-open="home"],#kfxHomePlanBtn').forEach(b=>text(b,'View / save my Keneflex Plan'));
 const next=document.querySelector('#solutionView .kfxKNext');if(next){text(next.querySelector('h2'),'Use the rest of your Keneflex Plan');const p=next.querySelector('.help');text(p,'Your product recommendations and personalized actions belong to one coordinated plan.');}
 const total=document.getElementById('total'),tb=total&&total.closest('.block');if(tb){const p=tb.querySelector('.help');text(p,'Product total. Your Keneflex Plan is part of the recommendation.');}
