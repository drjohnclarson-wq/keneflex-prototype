...
for(let i=0;i<500;i++){
 ...
 try{const s=E.createStore();E.ingest(s,story);const t=E.activeThread(s),q=E.nextQuestion(s);if(!t)throw Error('no active thread');if(side!=='both'&&t.side!==side)throw Error(`side lost: ${t.side}`);if(q&&E.known(t,q.concept))throw Error(`re-asked known fact: ${q.concept}`);if(/Which side is bothering you/i.test(q?.text||'')&&t.side)throw Error('redundant side question');if(/What does it feel like/i.test(q?.text||'')&&(t.symptoms.length||t.qualities.length))throw Error('generic symptom question despite known symptom');if(/What are you noticing there|Where is it centered most precisely|Where is the problem\?/i.test(q?.text||''))throw Error(`vague consumer question: ${q.text}`);pass++;}catch(e){failures.push({subject:i+1,story,error:e.message});}
}
