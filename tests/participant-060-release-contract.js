...
check(route.includes("searchParams.set('participant','065')"), 'participant route matches consolidated release');
check(route.includes("searchParams.set('build',String(stamp))"), 'participant route cache-busts each launch');
check(html.includes('participant-consolidated.css?v=065'), 'consolidated stylesheet is loaded');
