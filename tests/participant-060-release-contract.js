...
check(route.includes("searchParams.set('participant','060')"), 'participant route matches consolidated release');
check(route.includes("searchParams.set('build',String(stamp))"), 'participant route cache-busts each launch');
check(html.includes('participant-consolidated.css?v=060'), 'consolidated stylesheet is loaded');
...
check(controller.includes('fit, condition, cleanliness, and function'), 'owned products require suitability checks');
